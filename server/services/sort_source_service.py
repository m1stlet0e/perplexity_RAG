    ...
    def sort_sources(self, query:str,search_results:List[dict]):
        try:
            ...
            for res in search_results:
                ...
                similarity=float(np.dot(query_embedding,res_embedding)/(np.linalg.norm(query_embedding)*np.linalg.norm(res_embedding)))#calculating the similarity between the query and the content of the search results
